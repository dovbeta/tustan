...
<div class="ja-slidewrap<?php echo $skin_name;?><?php echo $params->get( 'moduleclass_sfx' );?> ja-<?php echo $source;?>" id="ja-slide-articles-<?php echo $module->id;?>" style="visibility:hidden">
 ...
 <!-- JA SLIDESHOW 3 MARK -->
  		 ...
  		 <div class="ja-slide-desc">
			 <a <?php echo $target; ?>  href="<?php echo   $item->link; ?>">
              ...
              <?php echo $helper->trimString( $item->introtext, $descMaxChars, $includeTags ); ?>
				...
				<a <?php echo $target; ?> class="readon readmore btn btn-mini btn-primary" href="<?php echo   $item->link; ?>">
  ...
  <?php endif; ?>
    ...
    <?php
    if($source == 'articles'){
        $catsid = $params->get('source-articles-display_model-modcats-category', '0');
        if(count($catsid) == 1 && $catsid[0] != ''){
            echo '<a class="btn-view-all btn btn-mini btn-primary" href="'.JRoute::_(ContentHelperRoute::getCategoryRoute($catsid[0])).'" title="'.JText::_('TPL_JA_MOD_CONTENTSLIDER_VIEW_ALL_TITLES').'">'.JText::_('TPL_JA_MOD_CONTENTSLIDER_VIEW_ALL').'</a>';
        }
    }elseif($source == 'com_k2'){
        $catsid = $params->get('k2catsid');
        if(count($catsid) == 1 && $catsid[0] != ''){
            echo '<a class="btn-view-all btn btn-mini btn-primary" href="'.urldecode(JRoute::_(K2HelperRoute::getCategoryRoute($catsid[0]))).'" title="'.JText::_('TPL_JA_MOD_CONTENTSLIDER_VIEW_ALL_TITLES').'">'.JText::_('TPL_JA_MOD_CONTENTSLIDER_VIEW_ALL').'</a>';
        }
    }
    ?>
