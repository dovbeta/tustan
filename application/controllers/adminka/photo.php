...
class Photo extends CI_Controller{
	...
	function upload_images($album='none',$whot_id='',$lang='ua'){
		...
		foreach($images as $img){
			...
			$img_id = $this->photo_model->add_photo($img_res[$i]['original'],$this->input->post('alt'),$img['size'],$this->data['user'][0]->id);
			if($album!='none'){
				$this->photo_model->add_photo_album($img_id,$album,$this->data['user'][0]->id);
			}
			$i++;
		}
