		...
		if (isset($item['vid'])) {
			switch ($item['mtype']) {
				case 'vimeo':
					...
					$context = '<div class="ja-slide-video" style="padding-bottom: ' . ($height / $width * 100) . '%"><iframe id="vimeo-' . JApplication::stringURLSafe($item['vid']) . '" src="' . $item['src'] . '" data-src="' . $item['src'] . '" frameborder="0" webkitAllowFullScreen mozallowfullscreen allowFullScreen></iframe></div>';
					break;
				case 'youtube':
		...
		if (!$this->isJson($jaGallery)) {
			...
			if (!empty($items)) {
				...
				for ($i = 0; $i < count($items); $i++) {
					if(!isset($items[$i]->show) || (isset($items[$i]->show) && $items[$i]->show == 'true')){
	...
	*/
	public function sortImage2($img1, $img2)
	{
		...
		if ($orderby == 1) {
			//Order by Time
			if ($orderby_type == 1) {
				return filemtime($folder . $img1->image) > filemtime($folder . $img2->image);
			} elseif ($orderby_type == 2) {
				return filemtime($folder . $img2->image) < filemtime($folder . $img1->image);
			}
		} else {
			//Order by Name
			if ($orderby_type == 1) {
